...
export default function TimerOverlay({ mandatorySeconds, maxExtraSeconds, onComplete, initialSeconds = 0, children }) {
  ...
  useEffect(() => {
    if (elapsedSeconds >= totalMax && !hasCalledOnComplete.current) {
      ...
      onComplete(elapsedSeconds, false, 'timer_expired');
      return;
    }
  ...
  }, [totalMax, onComplete, elapsedSeconds]);

  const handleEnd = (reason = null) => {
    clearInterval(intervalRef.current);
    hasCalledOnComplete.current = true;
    onComplete(elapsedSeconds, false, reason || 'user_ended');
  };

  const remainingSeconds = totalMax - elapsedSeconds;
  const mins = Math.floor(Math.max(0, remainingSeconds) / 60).toString().padStart(2, '0');
  const secs = (Math.max(0, remainingSeconds) % 60).toString().padStart(2, '0');
          ...
          <AnimatePresence>
            ...
            >
              ...
              <motion.button
                ...
                className="bg-red-600 hover:bg-red-700 text-white text-xs px-3 py-1 rounded-full transition whitespace-nowrap"
              >
                End Session
              </motion.button>
            </motion.div>
